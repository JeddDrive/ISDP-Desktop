...
namespace JeddoreISDPDesktop
{
    public partial class EditSupplierOrder : Form
    {
        ...
        public EditSupplierOrder(Employee employeeLoggedIn, Txn txnSentIn)
        {
            InitializeComponent();
            employee = employeeLoggedIn;

            //get the one txn (supplier order based on the ID)
            newOrder = TxnAccessor.GetOneTxn(txnSentIn.txnID);
        }

        private void EditSupplierOrder_Load(object sender, EventArgs e)
        {
            ...
            //add ALL SUPPLIERS as first item in the combobox
            cboSuppliers.Items.Add("ALL SUPPLIERS");

            //get a list of all of the suppliers
            List<Supplier> suppliersList = SupplierAccessor.GetAllSuppliersList();
        ...
        private void EditSupplierOrder_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            //call the show all supplier items ftn - setting up the other DGV
            ShowAllSupplierItems();
            ...
            txtSearchOrder.ResetText();

            //call update row colour ftn
            UpdateRowColour();

            //enable the suppliers combobox
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DialogResult btnValueReturned = MessageBox.Show("Are you sure you want to fully remove item of ID and name: " +
                theTxnItem.itemID + " - " + theTxnItem.name + " from your supplier order?", "Confirm Item Removal",
                ...
                if (btnValueReturned == DialogResult.Yes)
                {
                    ...
                    if (success)
                    {
                        MessageBox.Show("Item of ID and name: " + theTxnItem.itemID + " - " + theTxnItem.name + " has been fully removed from your supplier order.",
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (numTxnItems < 1)
            {
                MessageBox.Show("Supplier orders must contain at least one one item. " +
                    "You have " + numTxnItems.ToString() + " items. Please review your supplier order items.", "Supplier Order Not Submitted",
            ...
            if (success)
            {
                MessageBox.Show("Your supplier order has been successfully submitted to the suppliers.", "Successful Order Submission");
        ...
        private void btnViewItem_Click(object sender, EventArgs e)
        {
            ...
            if (selectedRowsCount != 1)
            {
                MessageBox.Show("Must select one row from the warehouse inventory data grid in order to view info about an individual item.",
                    "View Item Info Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            else
            {
                ...
                //then open and display the view item info form for the user
                //sending in the employee obj and selectedItem object
                ViewItem frmViewItem = new ViewItem(employee, selectedItem);
        ...
        //Show All Supplier Items - to display inventory items from all suppliers
        private void ShowAllSupplierItems()
        {
        ...
        private void ShowItemsBySupplier()
        {
            string[] stringArray = cboSuppliers.Text.Split(' ');

            //just need the first string from the array - the supplier ID
            int supplierID = Convert.ToInt32(stringArray[0]);

            //setting up the inventory DGV
            //create a new bindingsource
            BindingSource bindingSource2 = new BindingSource();

            //create datatable - getting all inventory from the warehouse for a specific supplier, returned as a datatable
            DataTable dt2 = InventoryAccessor.GetAllInventoryBySiteAndSupplierDataTable(2, supplierID);
            ...
            dgvInventory.Columns["siteID"].HeaderText = "Site ID";
            dgvInventory.Columns["quantity"].HeaderText = "Quantity";
            dgvInventory.Columns["supplierID"].HeaderText = "Supplier ID";
            //dgvInventory.Columns["itemLocation"].HeaderText = "Item Location";
            //dgvInventory.Columns["reorderThreshold"].HeaderText = "Reorder Threshold";
            //dgvInventory.Columns["optimumThreshold"].HeaderText = "Optimum Threshold";
            //dgvInventory.Columns["notes"].HeaderText = "Notes";
