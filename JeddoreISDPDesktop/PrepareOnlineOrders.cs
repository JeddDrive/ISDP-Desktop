...
{
    public partial class PrepareOnlineOrders : Form
    {
        ...
        public PrepareOnlineOrders(Employee employeeLoggedIn)
        {
            InitializeComponent();
            employee = employeeLoggedIn;
        }

        private void PrepareOnlineOrders_Load(object sender, EventArgs e)
        {
        ...
        {
            MessageBox.Show("This is the form for fulfilling and receiving online orders. You can select an online order here in the first tab, then click on order items to confirm their fulfillment in the second tab." +
                "\n\nClick on the 'refresh' button in either tab to load the appropriate data grid.", "Prepare Online Orders Help"
                , MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void PrepareOnlineOrders_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btnRefreshOrders_Click(object sender, EventArgs e)
        {
            ...
            //create datatable - getting all ONLINE orders for an employee's site, returned as a datatable
            dt = TxnAccessor.GetAllOnlineOrdersBySite(employee.siteID);
            ...
            dgvOrders.Columns["emergencyDelivery"].Visible = false;
            dgvOrders.Columns["destinationSite"].Visible = false;

            //change the header text of these columns
            dgvOrders.Columns["txnID"].HeaderText = "Order ID";
            dgvOrders.Columns["originSite"].HeaderText = "Site";
        ...
        private void txtSearchOrders_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dgvOrders.Rows)
                {
                    ...
                    //else if - shipped date cell converted to lower case contains the txtbox text
                    else if (notesCellValue != null && notesCellValue.ToString().ToLower().Contains(theSearchText))
                    {
                        row.Visible = true;
                    }
        ...
        private void btnSelectOrder_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //get the transaction
                Txn selectedTxn = TxnAccessor.GetOneTxn(txnID);

                //also assign the txnID to this global var
                selectedTxnID = txnID;

                //if the selected online order is not new or assembling
                if (selectedTxn.status != "New" && selectedTxn.status != "Assembling")
                {
                    MessageBox.Show("The status of the selected online order is " +
                        selectedTxn.status + ". An online order must be 'New' or 'Assembling' in order to be fulfilled.",
                        "Online Order Can't be Fulfilled", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (selectedTxn.txnID == txnID)
                {
                    ...
                    //if the online order status is 'New', then update it to 'Assembling'
                    if (selectedTxn.status == "New")
                    {
                        selectedTxn.status = "Assembling";

                        bool goodUpdate = TxnAccessor.UpdateTxnStatus(selectedTxn);
                    }

                    MessageBox.Show("Order " + selectedTxn.txnID.ToString() + " for " +
                        selectedTxn.destinationSite + " has been selected for fulfillment and now has a status of 'Assembling'. You can start " +
        ...
        private void btnRefreshOrderItems_Click(object sender, EventArgs e)
        {
            ...
            Txn selectedTxn = TxnAccessor.GetOneTxn(selectedTxnID);
        ...
        private void btnRemoveItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Must select one item in the listbox in order to remove it from being fulfilled for the online order.", "Item Removal Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            else if (lstFulfilledItems.SelectedIndex > 0)
            {
                ...
                foreach (TxnItems item in txnItemsList)
                {
                    ...
                    if (item.itemID == itemID)
                    {
                        ...
                        MessageBox.Show("Item " + item.itemID + " - " + item.name + " has been removed from being fulfilled in this online order.", "Item Removed From Fulfillment",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnAddItem_Click(object sender, EventArgs e)
        {
            ...
            if (selectedRowsCount != 1)
            {
                MessageBox.Show("Must select one row from the order items data grid in order to account for it being fulfilled in the online order.",
            ...
            else
            {
                ...
                foreach (TxnItems item in txnItemsList)
                {
                    ...
                    {
                        //display msg, clear the dgv, and exit the event
                        MessageBox.Show("Item is already assembled for this online order, and therefore it can't be added again.", "Item Already Assembled",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                //display msg, clear the dgv, and exit the event
                MessageBox.Show("Item " + txnItem.itemID + " - " + txnItem.name + " has been accounted for in fulfillment for this online order.", "Item Added to Order For Fulfillment",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    DialogResult btnValueReturned = MessageBox.Show("All items in this online order are accounted for now. Set order as being assembled now?",
                    ...
                    if (btnValueReturned == DialogResult.Yes)
                    {
                        ...
                        if (success)
                        {
                            MessageBox.Show("Online order for customer: \n\n" + selectedTxn.notes + " has been successfully fulfilled and is ready for delivery pickup. It's status has been updated to " + selectedTxn.status + ".",
        ...
        private void btnOrderFulfilled_Click(object sender, EventArgs e)
        {
            ...
            //if the real num of items in the order does not equal the 
            //count of the list of txnitems then
            if (numItemsInOrder != txnItemsList.Count)
            {
                MessageBox.Show("Not all items accounted for in this online order. Number of items required for this order is " +
                    numItemsInOrder.ToString() + ", while the number of items currently fulfilled in this order is " + txnItemsList.Count.ToString() + ".",
                    "Online Order is Missing Items", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void btnReceiveOrder_Click(object sender, EventArgs e)
        {
            ...
            if (selectedRowsCount != 1)
            {
                MessageBox.Show("Must select one row from the orders data grid in order to receive that online order.",
            ...
            else
            {
                ...
                {
                    MessageBox.Show("The status of the selected online order is " +
                        selectedTxn.status + ". An online order must be 'Assembled' in order to be received by the customer.",
                        "Online Order Can't be Received", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (selectedTxn.txnID == txnID)
                {
                    //open the receive online order form (modal), sending in the logged in employee and the selected online order
                    ReceiveOnlineOrder frmReceiveOnlineOrder = new ReceiveOnlineOrder(employee, selectedTxn);
                    frmReceiveOnlineOrder.ShowDialog();
                }
            }
        }
    }
